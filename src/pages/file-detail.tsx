...
export default function FileDetail() {
  ...
  const stored = fileId ? getFile(fileId) : undefined
  ...
  const organization = file
    ? getOrganization(file.organizationId)?.organization
    : undefined
  const user = file ? getOrganizationUser(file.organizationId) : undefined
  const linkedRecords = file ? recordsReferencingFile(file.id) : []
    ...
    <div className="flex min-w-0 flex-1 flex-col gap-6 overflow-x-hidden bg-muted/40 p-4 sm:p-6">
      {file ? (
        <>
          ...
          <section className="overflow-hidden rounded-2xl bg-card p-4 shadow-xs ring-1 ring-foreground/10 sm:p-5">
            ...
            {linkedRecords.length > 0 ? (
              <div className="flex flex-col gap-2">
                {linkedRecords.map((record) => {
                  const schema = getSchema(record.schemaId)?.schema
