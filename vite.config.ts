import path from "path"
import tailwindcss from "@tailwindcss/vite"
import react from "@vitejs/plugin-react"
import { defineConfig } from "vite"

const humaProxy = {
  target: "http://172.20.0.4:8000",
  changeOrigin: true,
}

// https://vite.dev/config/
export default defineConfig({
  plugins: [react(), tailwindcss()],
  resolve: {
    alias: {
      "@": path.resolve(import.meta.dirname, "./src"),
    },
  },
  server: {
    proxy: {
      "/auth": humaProxy,
      "/user": humaProxy,
      "/network": humaProxy,
      "/organization": humaProxy,
      "/organization-user": humaProxy,
      "/network-group": humaProxy,
      "/network-permission": humaProxy,
      "/network-member": humaProxy,
      "/organization-group": humaProxy,
      "/organization-permission": humaProxy,
      "/authorization": humaProxy,
      "/schema": humaProxy,
      "/workflow-definition": humaProxy,
      "/workflow": humaProxy,
      "/workflow-action": humaProxy,
      "/pipeline-definition": humaProxy,
      "/pipeline": humaProxy,
      "/pipeline-node": humaProxy,
      "/record": humaProxy,
      "/file": humaProxy,
      "/healthz": humaProxy,
      "/readyz": humaProxy,
    },
  },
})
